...
class OrderImportDetail extends Mapper implements \MVC\Domain\OrderImportDetailFinder {
    ...
    function __construct() {
		...
		$tblResource = "nhatrovanhuynh_resource";
		$tblOrderImport = "nhatrovanhuynh_order_import";
		$tblOrderImportDetail = "nhatrovanhuynh_order_import_detail";
		...
		$trackByExportStmt = sprintf("
			...
			from
				nhatrovanhuynh_session S inner join nhatrovanhuynh_session_detail SD on S.id = SD.idsession
			where
				SD.idcourse IN(select id_course from nhatrovanhuynh_r2c where id_resource=?) AND
